...
import { PairStats } from '@/data/analytics/types';
import { AnalyticsCard, PnlValue, WinRateValue, LowSampleBadge, type ColumnDef } from './AnalyticsCard';

interface PairsTableProps {
  bestPairs: PairStats[];
  worstPairs: PairStats[];
  bestSummary: string;
  worstSummary: string;
}

const pairColumns: ColumnDef<PairStats>[] = [
  { key: 'pair', label: 'Pair', render: (item) => (
    <div className="flex items-center gap-2">
      <span className="font-medium text-gray-900 dark:text-gray-100">{item.pair}</span>
...
export function PairsTable({ bestPairs, worstPairs, bestSummary, worstSummary }: PairsTableProps) {
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900/50">
      <div className="border-b border-gray-100 px-5 py-4 dark:border-gray-800">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <CoinsIcon className="h-4 w-4 text-gray-400 dark:text-gray-500" />
            <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-50">Pairs Performance</h2>
          </div>
      ...
      <AnalyticsCard
        title=""
        columns={pairColumns}
        data={view === 'best' ? bestPairs : worstPairs}
      />
    </div>
  );
}
